...
async function startServer() {
  ...
  const requireAdminSession = async (req: any, res: any, next: any) => {
    ...
    try {
      ...
      req.adminScope = admin.scope || "global";
      req.adminClientId = admin.client_id;
      next();
    } catch (err) {
  ...
  function applyAdminScope(query: any, req: any) {
    if (req.adminScope === "client" && req.adminClientId) {
      return query.eq("client_id", req.adminClientId);
    }
    return query;
  }
  ...
  app.post("/api/auth/send-otp", async (req, res) => {
    ...
    // 1. Check if number is associated with a client, client_user, or is a global admin
    const isGlobalAdminPhone = GLOBAL_ADMIN_PHONE && phone_e164 === normalizePhone(GLOBAL_ADMIN_PHONE);
    
    const { data: client } = await supabase.from("clients").select("id").eq("phone_e164", phone_e164).single();
    const { data: clientUser } = await supabase.from("client_users").select("id").eq("phone_e164", phone_e164).single();
    const { data: admin } = await supabase.from("admins").select("user_id").eq("phone_e164", phone_e164).single();

    if (!client && !clientUser && !admin && !isGlobalAdminPhone) {
      return res.status(404).json({ ok: false, error: "Este número não está associado a nenhum cliente." });
    }
  ...
  app.post("/api/auth/verify-otp", async (req, res) => {
    ...
    // 2. Check if user is an admin by phone
    const { data: admin } = await supabase.from("admins").select("*").eq("phone_e164", phone_e164).single();

    let client;
    let role: UserRole = 'visualizador';
    let userId;
    let isGlobalAdmin = isGlobalAdminPhone || (admin?.scope === "global");

    if (isGlobalAdmin && clientId) {
      // Global admin choosing a specific tenant
      const { data: c } = await supabase.from("clients").select("id, company_name, phone_e164").eq("id", clientId).single();
      if (!c) return res.status(404).json({ ok: false, error: "Cliente (tenant) não encontrado." });
      client = c;
      role = 'admin'; 
      userId = admin?.user_id || "global_admin";
    } else if (clientUser) {
      ...
      // 3. Check if user is the main client owner by phone
    ...
    if (!client && !isGlobalAdmin) return res.status(404).json({ ok: false, error: "Utilizador não registado." });
    
    // If global admin but no clientId provided, we require it to establish a tenant context
    if (isGlobalAdmin && !client) {
      return res.status(400).json({ ok: false, error: "Contexto de cliente (clientId) obrigatório para admin global." });
    }
    ...
    const token = jwt.sign({ 
      ...
      role,
      userId: userId || (admin?.user_id || "global_admin"),
    ...
    res.json({ 
      ok: true, 
      client: client ? { ...client, role, userId } : null,
      is_global_admin: isGlobalAdmin,
      can_act_as_admin: isGlobalAdmin,
      can_act_as_client: isGlobalAdmin,
      authenticated: true
    ...
    try {
      ...
      if (decoded.is_global_admin) {
        return res.json({
          ...
          phone_e164: decoded.phone_e164,
          is_global_admin: true,
          can_act_as_admin: true,
          can_act_as_client: true,
          role: decoded.role,
          userId: decoded.userId,
          can_access_restaurant_portal: true
        });
      }
      ...
      const hasRestaurantFinePerm = finePermissions?.some(p => p.module === 'restaurant' && p.actions.includes('view'));
      const rolePermissions = ROLE_PERMISSIONS[role];
      const hasRestaurantRolePerm = rolePermissions?.restaurant?.includes('view');
      
      const canAccessRestaurant = !!(hasRestaurantFinePerm || hasRestaurantRolePerm);
      ...
      res.json({ 
        ...
        role: decoded.role, 
        userId: decoded.userId,
        can_access_restaurant_portal: canAccessRestaurant,
        finePermissions: finePermissions || []
      });
    } catch {
      res.clearCookie("hub_session");
  ...
  app.get("/api/client/dashboard/stats", requireClientSession, async (req: any, res) => {
    ...
    try {
      ...
      const { data: tickets } = await supabase.from("tickets").select("status, kind").eq("client_id", clientId);
      ...
      const openTickets = tickets?.filter(t => ['aberto', 'novo', 'nova'].includes(t.status?.toLowerCase())).length || 0;
      const inProgressTickets = tickets?.filter(t => ['em análise', 'em investigação', 'em execução', 'a aguardar cliente'].includes(t.status?.toLowerCase())).length || 0;
      const resolvedTickets = tickets?.filter(t => ['concluído', 'resolvida', 'fechado ganho'].includes(t.status?.toLowerCase())).length || 0;
      const complaints = tickets?.filter(t => t.kind === 'reclamação').length || 0;
  ...
  app.get("/api/client/dashboard/charts", requireClientSession, async (req: any, res) => {
    const clientId = req.clientId;
    try {
      const { data: tickets } = await supabase.from("tickets").select("status, kind, created_at").eq("client_id", clientId);
      
      // Status Distribution
      const statusDistribution = {
        aberto: tickets?.filter(t => ['aberto', 'novo', 'nova'].includes(t.status?.toLowerCase())).length || 0,
        analise: tickets?.filter(t => ['em análise', 'em investigação', 'em execução'].includes(t.status?.toLowerCase())).length || 0,
        resolvido: tickets?.filter(t => ['concluído', 'resolvida', 'fechado ganho'].includes(t.status?.toLowerCase())).length || 0
      };

      // Type Distribution
      const typeDistribution = {
        pedido: tickets?.filter(t => t.kind === 'pedido').length || 0,
        reclamação: tickets?.filter(t => t.kind === 'reclamação').length || 0,
        outro: tickets?.filter(t => !['pedido', 'reclamação'].includes(t.kind)).length || 0
      };
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        daily.push({
          ...
          complaints: dayTickets?.filter(t => t.kind === 'reclamação').length || 0,
          resolved: dayTickets?.filter(t => ['concluído', 'resolvida'].includes(t.status?.toLowerCase())).length || 0
        });
      }
  ...
  // --- Travel Orders ---
  app.get("/api/admin/travel/orders", requireAdminSession, async (req: any, res) => {
    try {
      const { client_id, payment_status, sales_status, destination, search } = req.query;
      let query = supabase.from("travel_orders")
        .select("*, client:clients(company_name)")
        .order("created_at", { ascending: false });

      query = applyAdminScope(query, req);

      if (client_id) query = query.eq("client_id", client_id);
      if (payment_status) query = query.eq("payment_status", payment_status);
      if (sales_status) query = query.eq("sales_status", sales_status);
      if (destination) query = query.ilike("destination", `%${destination}%`);
      
      if (search) {
        query = query.or(`tracking_code.ilike.%${search}%,order_code.ilike.%${search}%,customer_name.ilike.%${search}%,destination.ilike.%${search}%`);
      }

      const { data: orders, error } = await query;
      if (error) throw error;

      res.json({ ok: true, orders: orders?.map(o => ({ ...o, company_name: (o.client as any)?.company_name })) });
    } catch (err: any) {
      res.status(500).json({ ok: false, error: err.message });
    }
  });

  app.get("/api/admin/travel/orders/:id", requireAdminSession, async (req: any, res) => {
    try {
      const { id } = req.params;
      let query = supabase.from("travel_orders")
        .select("*, client:clients(company_name), ticket:tickets(*)")
        .eq("id", id);
      ...
      res.json({ ok: true, order: { ...order, company_name: (order.client as any)?.company_name } });
    } catch (err: any) {
      res.status(500).json({ ok: false, error: err.message });
    }
  });

  app.post("/api/admin/travel/orders", requireAdminSession, async (req: any, res) => {
    try {
      const data = req.body;
      
      if (req.adminScope === "client" && data.client_id !== req.adminClientId) {
        return res.status(403).json({ ok: false, error: "Não tem permissão para criar pedidos para este cliente." });
      }

      if (data.amount_due === undefined || data.amount_due === null) {
        data.amount_due = (data.total_amount || 0) - (data.amount_paid || 0);
      }

      const { data: order, error } = await supabase.from("travel_orders").insert(data).select().single();
      if (error) throw error;
  ...
  app.patch("/api/admin/travel/orders/:id", requireAdminSession, async (req: any, res) => {
    try {
      const { id } = req.params;
      const updateData = req.body;

      let checkQuery = supabase.from("travel_orders").select("id, client_id").eq("id", id);
      checkQuery = applyAdminScope(checkQuery, req);
      const { data: existing } = await checkQuery.single();
      if (!existing) return res.status(404).json({ ok: false, error: "Pedido não encontrado ou sem acesso." });

      const { data: order, error } = await supabase.from("travel_orders").update(updateData).eq("id", id).select().single();
      if (error) throw error;
  ...
  // --- Travel Payments ---
  app.get("/api/admin/travel/payments", requireAdminSession, async (req: any, res) => {
    try {
      const { client_id, order_id, status, payment_method, search } = req.query;
      let query = supabase.from("travel_payments")
        .select("*, travel_orders(*), client:clients(company_name)")
        .order("created_at", { ascending: false });
      ...
      if (search) {
        query = query.or(`payment_code.ilike.%${search}%,external_reference.ilike.%${search}%,payer_name.ilike.%${search}%`);
      }

      const { data: payments, error } = await query;
      if (error) throw error;

      res.json({ ok: true, payments: payments?.map(p => ({ ...p, company_name: (p.client as any)?.company_name })) });
    } catch (err: any) {
      res.status(500).json({ ok: false, error: err.message });
    }
  });

  app.get("/api/admin/travel/payments/:id", requireAdminSession, async (req: any, res) => {
    try {
      const { id } = req.params;
      let query = supabase.from("travel_payments")
        .select("*, travel_orders(*), client:clients(company_name)")
      ...
      res.json({ ok: true, payment: { ...payment, company_name: (payment.client as any)?.company_name } });
    } catch (err: any) {
      res.status(500).json({ ok: false, error: err.message });
    }
  });

  app.post("/api/admin/travel/payments", requireAdminSession, async (req: any, res) => {
    try {
      const data = req.body;
      
      if (req.adminScope === "client" && data.client_id !== req.adminClientId) {
        return res.status(403).json({ ok: false, error: "Não tem permissão para criar pagamentos para este cliente." });
      }

      const { data: payment, error } = await supabase.from("travel_payments").insert(data).select().single();
      if (error) throw error;
  ...
  app.patch("/api/admin/travel/payments/:id", requireAdminSession, async (req: any, res) => {
    try {
      const { id } = req.params;
      const updateData = req.body;

      let checkQuery = supabase.from("travel_payments").select("id, client_id").eq("id", id);
      checkQuery = applyAdminScope(checkQuery, req);
      const { data: existing } = await checkQuery.single();
      if (!existing) return res.status(404).json({ ok: false, error: "Pagamento não encontrado ou sem acesso." });

      const { data: payment, error } = await supabase.from("travel_payments").update(updateData).eq("id", id).select().single();
      if (error) throw error;
  ...
  // --- Travel Stats ---
  app.get("/api/admin/travel/stats", requireAdminSession, async (req: any, res) => {
    try {
      let ordersQuery = supabase.from("travel_orders").select("sales_status, payment_status, amount_due");
      let paymentsQuery = supabase.from("travel_payments").select("amount, status, payment_method, payment_type");

      ordersQuery = applyAdminScope(ordersQuery, req);
      paymentsQuery = applyAdminScope(paymentsQuery, req);
      ...
      const stats = {
        ...
        quotedOrders: orders?.filter(o => o.sales_status === 'quoted').length || 0,
        awaitingPaymentOrders: orders?.filter(o => o.sales_status === 'awaiting_payment').length || 0,
        confirmedOrders: orders?.filter(o => o.sales_status === 'confirmed').length || 0,
        completedOrders: orders?.filter(o => o.sales_status === 'completed').length || 0,
        cancelledOrders: orders?.filter(o => o.sales_status === 'cancelled').length || 0,
        
        pendingPayments: payments?.filter(p => p.status === 'pending').length || 0,
        paidPayments: payments?.filter(p => p.status === 'paid').length || 0,
        
        partialOrders: orders?.filter(o => o.payment_status === 'partial').length || 0,
        
        totalPaidAmount: payments?.filter(p => p.status === 'paid').reduce((acc, p) => acc + (p.amount || 0), 0) || 0,
        totalOutstandingAmount: orders?.reduce((acc, o) => acc + (o.amount_due || 0), 0) || 0,
        
        pixPayments: payments?.filter(p => p.payment_method === 'pix').length || 0,
        cardPayments: payments?.filter(p => p.payment_method === 'credit_card' || p.payment_method === 'debit_card').length || 0,
        installmentPayments: payments?.filter(p => p.payment_type === 'installment').length || 0,
        boletoPayments: payments?.filter(p => p.payment_method === 'boleto').length || 0
      };
  ...
  // --- Client Travel Data ---
  app.get("/api/admin/clients/:id/travel-orders", requireAdminSession, async (req: any, res) => {
    try {
      const { id } = req.params;
      
      if (req.adminScope === "client" && id !== req.adminClientId) {
        return res.status(403).json({ ok: false, error: "Acesso negado a este cliente." });
      }

      const { data: orders, error } = await supabase.from("travel_orders")
        .select("*")
        .eq("client_id", id)
        .order("created_at", { ascending: false });
  ...
  app.get("/api/admin/clients/:id/travel-payments", requireAdminSession, async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      const { data: payments, error } = await supabase.from("travel_payments")
        .select("*, travel_orders(order_code)")
        .eq("client_id", id)
        .order("created_at", { ascending: false });
