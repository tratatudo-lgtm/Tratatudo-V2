...
async function startServer() {
  ...
  app.get("/", (req, res) => {
    res.send("TrataTudo API running ✅");
  });
  ...
  app.post("/api/auth/send-otp", async (req, res) => {
    try {
      const rawPhone = String(req.body?.phone_e164 || req.body?.phone || req.body?.number || "").trim();
      ...
      const phone_e164 = "+" + digits;

      if (!phone_e164.startsWith("+351") || phone_e164.length < 13) {
      ...
      if (!EVO_URL || !EVO_KEY) {
        return res.status(500).json({ ok: false, error: "EVO_URL/EVO_KEY em falta." });
      }
      ...
      try {
        ...
        if (!sendRes.ok) {
          ...
          console.error("OTP WhatsApp send failed:", sendRes.status, sendBody);
          return res.status(502).json({ ok: false, error: "Falha ao enviar código por WhatsApp." });
        }
      } catch (sendErr) {
        console.error("OTP WhatsApp send error:", sendErr);
        return res.status(502).json({ ok: false, error: "Falha ao enviar código por WhatsApp." });
      }
  ...
  app.post("/api/bot/reply", async (req, res) => {
    try {
      const apiKey = req.headers["x-tratatudo-key"];
      const expectedKey = process.env.ADMIN_API_KEY || process.env.TRATATUDO_API_KEY || "";
      ...
      const isServiceActive = (client: any) => {
        if (!client) return false;
        const status = safeStr(client.status).toLowerCase();
        if (status === "trial") {
      ...
      const extractAndStripReport = (fullText: string) => {
      ...
      const basePrompt = String(process.env.SYSTEM_BASE_PROMPT || "").trim();
      const finalPrompt = [basePrompt, safeStr(client?.bot_instructions)].filter(Boolean).join("\n\n").trim();

      if (!finalPrompt) {
        return res.status(500).json({
          ok: false,
          error: "Prompt vazio. Define SYSTEM_BASE_PROMPT ou bot_instructions."
        });
      }
      ...
      let historyMessages = (history || []).reverse().map((m: any) => ({
      ...
      if (
        ...
        String(historyMessages[historyMessages.length - 1]?.content || "").trim() === String(text || "").trim()
      ) {
      ...
      const instance = safeStr(instance_name || client?.instance_name);

      let finalReply = replyClean;
      ...
      if (report) {
        const { data: insertedTicket, error: ticketErr } = await supabase
          ...
          .insert([{
            client_id,
            kind: report.type === "complaint" ? "reclamação" : "pedido",
            category: safeStr(report.category),
            subject: safeStr(report.subject) || null,
            description: safeStr(report.description || report.details || text),
          ...
          .select("id, tracking_code, kind")
          .single();

        if (ticketErr) throw ticketErr;

        if (insertedTicket?.tracking_code) {
          finalReply =
            insertedTicket.kind === "reclamação"
              ? `Feito! ✅ Registei a reclamação com o código ${insertedTicket.tracking_code}. Podes perguntar a qualquer momento sobre o estado da reclamação.`
              : `Feito! ✅ Registei o pedido com o código ${insertedTicket.tracking_code}. Podes perguntar a qualquer momento sobre o estado do pedido.`;
        }
      }
      ...
      await supabase.from("wa_messages").insert([
        {
          ...
          direction: "outbound",
          text: finalReply,
          raw: { model, source: "groq", push_name, had_report: !!report }
        }
      ]);
