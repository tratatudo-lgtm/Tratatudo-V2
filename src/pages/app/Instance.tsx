...
export function Instance() {
            ...
            const stats: Stats = result?.stats
              ? {
                  totalMessages:
                    ...
                    ((result.stats.sentMessages || result.stats.messages_sent || 0) +
                      (result.stats.receivedMessages || result.stats.messages_received || 0)),
  ...
  const syncInstance = async () => {
    try {
      ...
      if (!res.ok) {
        throw new Error('Falha ao sincronizar instância.');
      }

      await fetchInstanceData();
    } catch (err: any) {
      console.error('[APP] Sync failed:', err);
      setError(err.message || 'Erro ao sincronizar instância.');
    } finally {
      setSyncing(false);
    }
  };
  ...
  const quickStats = [
    {
      ...
      value: stats.receivedMessages.toLocaleString(),
      icon: MessageSquare,
      color: 'text-blue-500',
      bg: 'bg-blue-50'
    },
          ...
          <div className="p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-900">{instance.instance_name || 'Instância'}</h2>
                  <p className="text-slate-500 font-mono text-sm">{instance.whatsapp_number || 'N/A'}</p>
                </div>
              </div>
              ...
              <div className="flex flex-col items-end gap-2">
                <div
                  className={cn(
                    ...
                    (instance.status || '').toLowerCase() === 'conectado'
                      ? 'bg-emerald-50 text-emerald-600 border border-emerald-100'
                      : (instance.status || '').toLowerCase() === 'reconectando'
