  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2 p-1 bg-slate-100 rounded-2xl border border-slate-200">
        {(Object.keys(AREA_CONFIG) as OperationalArea[]).map((area) => {
          const config = AREA_CONFIG[area];
          const Icon = area === 'pedidos' ? ClipboardList : area === 'reclamacoes' ? AlertCircle : TrendingUp;
          if (!canSee(area)) return null;
          return (
            <button
              key={area}
              onClick={() => { setSearchParams({ area }); setActiveArea(area); }}
              className={cn(
                "flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm transition-all",
                activeArea === area ? `bg-white ${config.textMain} shadow-sm` : "text-slate-500 hover:bg-slate-50"
              )}
            >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-display font-bold text-slate-900">Hub de {AREA_CONFIG[activeArea].label}</h1>
          <p className="text-slate-500 text-sm">Gestão operacional em tempo real.</p>
        </div>
        {can(activeArea, 'criar') && (
          <button 
            onClick={() => setIsSupportOpen(true)}
            className={cn("w-full sm:w-auto px-6 py-3 rounded-xl font-bold text-sm text-white shadow-lg flex items-center justify-center gap-2", AREA_CONFIG[activeArea].bgMain)}
          >
