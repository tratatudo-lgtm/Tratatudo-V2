...
// Inicializa o import { useEffect, useState } from 'react';
import { createClient } from '@supabase/supabase-js';
...
// Define o URL base onde a tua Evolution API está alojada
const EVOLUTION_API_URL = 'http://o_ip_da_tua_vps:8080'; 
...
export default function UltimateSaaSDashboard() {
  ...
  useEffect(() => {
    ...
    // ⚡ CANAL REALTIME CENTRALIZADO
    const channel = supabase
      .channel('saas_realtime_hub')
      .on('postgres_changes', { event: '*', pattern: 'public', table: 'wa_chats' }, (p) => {
        setChats(prev => prev.map(c => c.id === p.new.id ? { ...c, ...p.new } : c));
        // Injeta um log em direto no painel para dar o efeito avançado
  ...
  // 🔌 INTERAÇÃO COM A EVOLUTION API: Ir buscar o QR Code real
  const fetchQRCode = async (instanceName: string, token: string) => {
    ...
    try {
      ...
      if (response.data?.base64) {
        setQrCode(response.data.base64); // Grava a string base64 da imagem do QR Code
      } else {
        setQrCode('CONNECTED'); // Se não der QR, é porque já está ligado!
      }
    } catch (err) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 mt-8">
        ...
        {activeTab === 'chats' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            {/* Listagem de Chats (Ocupa 2 colunas) */}
              ...
              <div className="grid grid-cols-1 gap-4">
                {chats.map(chat => (
                  <div key={chat.id} className="bg-gradient-to-b from-slate-900 to-slate-900/60 border border-slate-800/80 rounded-2xl p-5 hover:border-slate-700 transition-all shadow-xl group">
                    ...
                    {/* Dados coligidos pela inteligência artificial */}
            ...
            {/* Terminal Hacker de Eventos IA (A joia da coroa do painel) */}
            <div className="space-y-4">
              ...
              <div className="bg-black border border-slate-800 rounded-2xl p-4 font-mono text-[11px] text-cyan-400 h-[450px] overflow-y-auto space-y-2 shadow-inner shadow-cyan-500/5">
                <div className="text-slate-500 text-center border-b border-slate-900 pb-2 mb-2">--- ESCUTA ATIVA DE WEBHOOKS (SUPABASE REALTIME) ---</div>
                {aiLogs.map((log, index) => (
                  <div key={index} className="leading-relaxed whitespace-pre-wrap animate-fade-in text-slate-300">
                    <span className="text-indigo-400">➔</span> {log}
                  </div>
                ))}
                {aiLogs.length === 0 && <div className="text-slate-600 italic text-center pt-8">Nenhuma atividade registada nesta sessão. Aguarda interações...</div>}
              </div>
            </div>
        ...
        {/* TAB 2: CLIENTES, MASTER PROMPTS E INFRAESTRUTURA WHATSAPP */}
        {activeTab === 'instances' && (
          <div className="space-y-6">
            ...
            {/* Super Modal de Controlo da Instância (Configuração de Prompt e QR Code) */}
            {selectedInstance && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl relative space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Coluna Esquerda: Edição do Prompt de IA do Cliente */}
                    <div className="space-y-3">
                      <label className="block text-xs font-mono uppercase tracking-wider text-slate-400">🧠 Comportamento e Prompt do Robô (Master Prompt)</label>
                      <textarea
                        value={editingPrompt}
                        onChange={(e) => setEditingPrompt(e.target.value)}
                        placeholder="Ex: És o atendente comercial do ginásio X. Oferece o plano anual..."
                    ...
                    {/* Coluna Direita: QR Code Vivo da Evolution API */}
                    <div className="space-y-3 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500 mt-1">Conecta o telemóvel comercial do teu cliente à infraestrutura SaaS.</p>
                      </div>
                      ...
                      <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col items-center justify-center text-center min-h-[220px]">
                        ...
                        {!loadingQr && qrCode && qrCode !== 'CONNECTED' && qrCode !== 'ERROR' && (
                          <>
                            ...
                            <p className="text-[10px] text-slate-400 mt-3 font-mono">Lê o código no WhatsApp do smartphone do cliente.</p>
                          </>
                        )}
                        ...
                        {!loadingQr && qrCode === 'CONNECTED' && (
                          <div className="text-center text-xs font-mono text-emerald-400 space-y-2">
                            ...
                            <p className="text-[10px] text-slate-500">O robô já consegue ler e responder mensagens.</p>
                          </div>
                        )}

                        {!loadingQr && (!qrCode || qrCode === 'ERROR') && (
                          <p className="text-xs text-rose-400 font-mono italic">Falha ao obter QR Code. Verifica os logs da tua VPS.</p>
                        )}
                      </div>
                      ...
                      >
                        🔄 Gerar Novo QR Code / Atualizar Status
                      </button>
                    </div>
                  </div>
 ...
 do Supabase (Ajusta com as tuas variáveis de ambiente do Next/Vite)
