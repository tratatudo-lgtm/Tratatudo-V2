...
export default function UltimateSaaSDashboard() {
  ...
  // 🛡️ VERIFICAÇÃO DE SESSÃO ADMIN
  useEffect(() => {
    async function checkUser() {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) {
        // Se não houver token/sessão ativa, chuta para a página de login
        router.push('/admin/login');
      } else {
        // Se estiver autenticado, carrega a infraestrutura normal
        loadInitialData();
      }
    }
    checkUser();
  ...
  useEffect(() => {
    // Escuta Realtime (Apenas se o activeChat mudar)
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-6 flex flex-col">
      {/* Navbar Premium com botão de Logout */}
 ...
 o cliente do Supabase no Frontend usando as tuas variáveis de ambiente
