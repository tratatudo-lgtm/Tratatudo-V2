...
anchor = 'app.post("/api/admin/clients/:id/activate-production", requireAdminSession, async (req: any, res: any) => {'
if anchor not in text:
    raise SystemExit("ANCHOR_NOT_FOUND")

block = '''app.post("/api/admin/clients/:id/prolong-trial", requireAdminSession, async (req: any, res: any) => {
  try {
    const { id } = req.params;
